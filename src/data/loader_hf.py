...
class LoaderHF:
    def __init__(self, 
        ...
        **kwargs
        ...
        dataset['test'] = dataset['val']#dataset['train'].filter(lambda x, idx: idx % 10 == 0, with_indices=True)
        #dataset['test'] = dataset['val'].filter(lambda x, idx: idx % 2 == 0, with_indices=True)
        #dataset['val'] = dataset['val'].filter(lambda x, idx: idx % 10 == 0, with_indices=True)
