...
train_size = (
    total_rows - val_size - test_size if test_size > 0 else total_rows - val_size
)
...
lance.write_dataset(
    generate_record_batches(ds, train_indices, chunk_size=CHUNK_SIZE),
    train_dataset_path,
    schema=ds.schema,
)
lance.write_dataset(
    generate_record_batches(ds, val_indices, chunk_size=CHUNK_SIZE),
    val_dataset_path,
    schema=ds.schema,
)
...
if test_size > 0:
    lance.write_dataset(
        generate_record_batches(ds, test_indices, chunk_size=CHUNK_SIZE),
        test_dataset_path,
        schema=ds.schema,
    )
