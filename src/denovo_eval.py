""" From https://github.com/instadeepai/InstaNovo/blob/main/instanovo/utils/metrics.py
"""
...
class Metrics:
    ...
    def __init__(
        self,
        residues: dict[str, float] = RESIDUES,
        isotope_error_range: list[int] = [0, 1],
    ...
    def _split_sequences(seq: list[str] | list[list[str]]) -> list[list[str]]:
        return [
            re.split(r"(?<=.)(?=[A-Z])", x) if isinstance(x, str) else x for x in seq
        ]
    ...
    def compute_aa_er(
        ...
        return float(
            jiwer.wer(
                [" ".join(x) for x in peptides_truth],
                [" ".join(x) for x in peptides_predicted],
            )
        )
