...
def dc_decoder_base(
    amod_dict, d_model=256, dropout=0, cross_attend=True, max_seq_len=31, **kwargs
):
    model = PeptideTransformerDecoder(
        amod_dict,
        d_model,
...
def dc_decoder_deeper(
    amod_dict, d_model=256, dropout=0.25, cross_attend=True, **kwargs
):
    model = PeptideTransformerDecoder(
        amod_dict,
        d_model,
...
def dc_decoder_jl(
    amod_dict, d_model=256, dropout=0.1, cross_attend=True, max_seq_len=100, **kwargs
):
    model = PeptideTransformerDecoder(
        amod_dict,
        d_model,
...
def dc_casanovo_decoder(
    amod_dict, d_model=512, dropout=0, cross_attend=True, max_seq_len=100, **kwargs
):
    model = PeptideTransformerDecoder(
        amod_dict,
        d_model,
